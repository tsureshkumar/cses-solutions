...
int main(int argc, char **argv) {
    ...
    repr(i,1,n+1) {
        ...
        repr(j,1,sum+1) {
            dp[j][cur] = dp[j][other];
            int coin = v[i];
            if(j==coin) dp[j][cur] = 1;
            else if(j-coin >= 0) dp[j][cur] |= dp[j-coin][other];

            if(i==n && dp[j][cur]) res.pb(j);
        }
    }

    // setw(3); put("*"); rep(i,n+1) {setw(3); put(v[i]); } putl();
    // rep(i,sum+1) {
    //     setw(3); put(i);
    //     rep(j,n+1) { setw(3); put(dp[i][j]);}
    //     putl();
    // }

    sort(all(res));
    putl(res.size());
    for(auto x : res) put(x);
    putl();
