...
int main(int argc, char **argv) {
    ...
    vi level(n, INF);
    level[0] = 0;
    rep(i,n) jump[i][0] = par[i], level[i] = min(level[i], level[par[i]] + 1);
    repr(k,1,20) {
        rep(i,n) {
            jump[i][k] = jump[jump[i][k-1]][k-1];
            int parent = jump[i][k];
            level[i] = min(level[i], level[parent] + (1<<k));
        }
    }

    // auto find_level = [&](int x) {
    //     int res = 0;
    //     while(x != 0) {
    //         dwn(k,20) {
    //             if(jump[x][k] != 0) {
    ...
    auto lca = [&](int x, int y) {
        ...
        int xlvl = level[x], ylvl = level[y];

        if(xlvl > ylvl) swap(x, y);

        y = mkjump(y, abs(xlvl-ylvl));

        if(x == y) return x;

        // should be at same level for this to terminate
        while(x != y) {
            dwn(i,20) {
                int u = jump[x][i];
                int v = jump[y][i];
