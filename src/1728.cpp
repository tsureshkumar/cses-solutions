...
int main(int argc, char **argv) {
    ...
    int n; see(n);
    vi r(n); seea(r, 0, n); // r[i] is range (1..r[i]) from where x[i] is drawn

    // E(inversions) = sum(P(x[i] > x[j])) for i < j
    // x[i] is drawn from range [1..r[i]]
    // x[j] is drawn from range [1..r[j]]
    // P(x[i] > X[j]) = #(u,v) s.t. u < v and x[i] > x[j] / r[i] * r[j]
    // #(u,v) s.t. u < v = sum (r[i] - v) for v = 1 to r[j]
    // P(x[i] > x[j]) = 1 - P(x[i] <= x[j]) = 1 - (x[j] + 1) / (2 * x[i])
    auto solve = [&]() {
        LD E = 0.0;
        rep(i, n) {
            repr(j, i+1, n) {
                if(r[i] > r[j]) {
                    E += ((LD)1 - (r[j] + 1) / ((LD)2.0 * r[i]));
                } else {
                    E += (r[i] - 1.0) / ((LD)2.0 * r[j]);
                }
            }
        }
        return E;
