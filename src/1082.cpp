...
LL sigma(LL n) {
    ...
    for(LL i=2; i*i<=n; i++) {
        if(n%i==0) {
            LL c = 0;  
            while(n%i==0) n/=i, c++;
            res = (res*(modpow(i, c+1, MD)-1))%MD;
            res = (res*modpow(i-1, MD-2, MD))%MD;
        }
    }
    if(n > 1) {
        res = (res*(modpow(n, 2, MD)-1))%MD;
        res = (res*modpow(n-1, MD-2, MD))%MD;
    }
    return res;
}
...
int main(int argc, char **argv) {
    ...
    auto solve = [&](LL n) -> LL {
        LL res = 0;
        LL div2 = modpow(2, MD-2, MD);
        LL c, right;
        for(LL k=1; k<=n; k=right+1) {
            // debug(n, k, n/k, k*(n/k));

            c = n/k;
            right = n/c; // n / floor(n/k) gives the rightmost k where c repeats

            // sum the AP from k to right
            LL len = (right-k+1) % MD;
            LL sum = ((k + right) % MD);
            sum = (sum * div2) % MD;
