...
int main(int argc, char **argv) {
    ...
    // augment the flow from S to T using Edmonds-Karp
    function<LL(int, int)> augment = [&](int S, int T) {
        ...
        cur = T;
        while(cur != S) {
            cap[par[cur]][cur] -= bottleneck;
            cap[cur][par[cur]] += bottleneck;
            cur = par[cur];
        }
        return bottleneck;
    };
    ...
    while(!q.empty()) {
        int u = q.front(); q.pop();
        for(auto v : adj[u]) {
            if(reachable[v] || cap[u][v] <= 0) continue;
            reachable[v] = 1;
            q.push(v);
        }
    }
    ...
    rep(i, n) {
        ...
        for(auto v : adj[i]) {
            if(!reachable[v] && cap[i][v] == 0) cut.pb({i, v});
        }
    }

    putl(cut.size());
    for(auto u : cut) putl(u.fi+1, u.se+1);
