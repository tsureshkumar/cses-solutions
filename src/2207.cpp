...
int mex(ordered_set &s) {
    int left = 0, right = s.size();
    while(left < right) {
        int mid = (left + right) / 2;
        if(*s.find_by_order(mid) == mid) left = mid + 1;
        else right = mid;
    }
    return left;
}
...
int main(int argc, char **argv) {
    ...
    // check range, as mex(g(i)^g(n-i)) can't be further improved beyond quadratic
    // int mn = 1e9, mx = 0;   
    // repr(i, 2000, 3000) {
    //     mn = min(mn, grundy(i));
    //     mx = max(mx, grundy(i));
