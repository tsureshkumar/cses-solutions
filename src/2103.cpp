...
int main(int argc, char **argv) {
    ...
    for(int k=1; k<n; k*=2) {
        ...
        ra[sa[0]] = 0;
        repr(i,1,n) ra[sa[i]] = i > 0 && pairs[sa[i]] == pairs[sa[i-1]] ? ra[sa[i-1]] : i;
    }

    vi lcp(n, 0), lcp_ra(n, 0);
    rep(i,n) lcp_ra[sa[i]] = i;
    int k = 0;
    rep(i,n) {
        if(lcp_ra[i] == n-1) {
            k = max(k-1, 0);
    ...
    #if DEBUG == 1
    rep(i,n) setw(3), putl(sa[i], ra[i], lcp_ra[i], lcp[i], t.substr(sa[i]));
    #endif
