...
int main(int argc, char **argv) {
    ...
    // greedy strategy says the relative reward depends only on duration (shortest time rule)
    // t1: d1 D1
    // t2: d2 D2
    // reward 1 = D1 - d1 + D2 - d1 - d2
    // reward 2 = D2 - d2 + D1 - d1 - d2
    // relative reward 1 - 2 = d2-d1. 

    sort(all(tasks), [&](ii x, ii y) {
      int r1 = x.se - x.fi, r2 = y.se - y.fi;  
      if(x.fi == y.fi) return r1 < r2;
      return x.fi < y.fi;
    });

    LL clock = 0, rewards = 0;
    for(auto [duration, deadline]: tasks) {
       clock += duration; 
       rewards += (LL)deadline - clock;
    }
