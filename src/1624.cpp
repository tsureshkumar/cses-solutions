...
int res=0;
vvi board(8, vi(8, 0));
vi rows(8, 0), cols(8, 0), diag1(20, 0), diag2(20, 0);
...
void printboard() {
    ...
    rep(i,8) {
        rep(j,8) {
            if(board[i][j] == -1) put("_");
            else if(board[i][j]==1) put("Q");
            else put(".");
        }
        putl();
    }
...
void btrack(int n, int i, int j) {

    // previous col should have filled
    if(j>=1 && cols[j-1] == 0)  return;
    if(j>=7 && i-1>=0 && rows[i-1] == 0) return;
    ...
    int nxti=(i+1)%8, nxtj=i+1>=8?j+1:j;

    btrack(n, nxti, nxtj);

    if(board[i][j] == -1 || rows[i] != 0|| cols[j] != 0  || diag1[(j-i+8-1)]  != 0 || diag2[i+j] != 0 ) {
        return;
    }


    // place queen here
    rows[i] = cols[j] = diag1[j-i+8-1] = diag2[i+j] = 1;
    board[i][j] = 1;
    btrack(n-1, nxti, nxtj);
    board[i][j] = 0;
    rows[i] = cols[j] = diag1[j-i+8-1] = diag2[i+j] = 0;
