...
int main(int argc, char **argv) {
    ...
    // returns (max matching with u, max matching without u)
    struct node {
        int with, without;
    };
    function<node(int, int)> dfs = [&](int u, int p) {
        node res = {0, 0};
        for(int v: adj[u]) {
            if(v == p) continue;
            node child = dfs(v, u);
            ...
            node prev = res;
            res.with = max(prev.without + child.without + 1, prev.with + max(child.without, child.with));
            res.without += max(child.without, child.with);
        }
    ...
    node res = dfs(0, -1);
    debug(res.with, res.without);
    putl(max(res.with, res.without));
