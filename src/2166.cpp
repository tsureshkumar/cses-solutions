...
int main(int argc, char **argv) {
    ...
    const int N = 1<<(32-__builtin_clz(n));
    vector<ll> seg(2*N, {0, 0});
    debug(N);
    ...
    auto update = [&](int pos, int v) {
        ...
        seg[pos] = {v, v};
        for(i=pos/2; i > 0; i /= 2) {
            debug(i, 2*i, 2*i+1, seg.sz);
            seg[i].fi = seg[i*2].fi + seg[i*2+1].fi;

            LL best = seg[i].fi; // total
            best = max(best, seg[2*i].se); // prefix on left
            best = max(best, seg[2*i].fi + seg[2*i+1].se); // sum on left + prefix on right

            seg[i] = {seg[i].fi, best};
        }
        debug(seg[1].fi, seg[1].se);
    };
    ...
    auto printseg = [&]() {
        ...
        rep(i, 2*N) {
            ...
            put("[", seg[i].fi, ", ", seg[i].se, "]");
        }
        putl("");
        #endif
    };

    function<ll(int,int,int,int,int)> max_prefix_sum = [&](int i, int l, int r, int a, int b ) -> ll {
        debug(i, l, r, a, b);
        if(l > b || r < a) return {-INF, -INF};
        if(a <= l && r <= b) return seg[i];

        int m = (l+r)/2;
        ll left = max_prefix_sum(i*2, l, m, a, b);
        ll right = max_prefix_sum(i*2+1, m+1, r, a, b);

        if(left.fi == -INF) return right;
        if(right.fi == -INF) return left;

        LL total = left.fi + right.fi;
        LL best = total; // total
        best = max(best, left.se); // prefix on left
        best = max(best, left.fi + right.se); // sum on left + prefix on right
    ...
    while(q--) {
        int c, x, y; see(c, x, y);
        if(c == 2) {
            debug("querying", x-1, y-1);
            LL res = max_prefix_sum(1, 0, N-1, x-1, y-1).se;
            putl(max(0LL, res));
        } else {
            debug("updating", x-1, y);
            update(x-1, y);
            printseg();
        }
    }
