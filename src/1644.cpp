...
int main(int argc, char **argv) {
    ...
    /*
    approach: calculate the max sum of subarray ending at i with length between a and b.
    to do this, if we substract current prefix sum with some other prefix sum within the 
    range [i-b, i-a], we get the max at i.
    so, we need algorithm to find minimum of a range. use monotonic queue of length k.
    */

    LL res = -INF;
    deque<LL> dq;
    dq.push_back(0);  // Start with 0th index
    
    repr(i, a, n+1) {
        // Remove elements outside the window [i-b, i-a]
        ...
        // Maintain monotonic queue for minimum prefix sum
        while(!dq.empty() && prefix[dq.back()] >= prefix[i-a+1])
            dq.pop_back();
        dq.push_back(i-a+1);
    }
