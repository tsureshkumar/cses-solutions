...
int main(int argc, char **argv) {
    ...
    vi vis(n, 0), par(n, -1);
    function<int(int, int, int)> eulerTour = [&](int u, int p, int itr) {

        if(vis[u] == itr) {
            return u;
        } else if (vis[u] > 0 && vis[u] < itr) { return -1;  }
        vis[u] = itr;
        
        for(int v: adj[u]) {
            if(v == p) continue;
            par[v] = u;
            int res = eulerTour(v, u, itr);
            if(res != -1) return res;
        }
