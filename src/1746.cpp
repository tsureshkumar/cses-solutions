...
int main(int argc, char **argv) {
    ...
    dwnr(i, 1, n) {
        if(arr[i] == 0) {
            repr(j, 1, m+1) {
                (dp[i][j] = dp[i+1][j] + dp[i+1][j-1] + dp[i+1][j+1]) %= MD;
            }
        } else {
            (dp[i][arr[i]] = dp[i+1][arr[i]] + dp[i+1][arr[i]-1] + dp[i+1][arr[i]+1])%=MD;
        }
    }
    ...
    // setw(4); put("*"); rep(j,n+2) {setw(4); put(j);} putl();
    // setw(4); put("a"); rep(j,n+2) {setw(4); put(arr[j]);} putl();
    // rep(i, m+1) {
    //     setw(4); put(i);
    //     rep(j, n+2) {
    //         setw(4); put(dp[j][i]);
