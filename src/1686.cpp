...
int main(int argc, char **argv) {
    ...
    while(!st.empty()) {
        int u = st.top(); st.pop();
        if(!vis[u]) dfs2(u, scc_count++);
    }

    vector<set<int>> condensed(scc_count);
    vl condensed_weights(scc_count);
    rep(i,n) {
        condensed_weights[scc[i]] += node_weights[i];
        for(int v: adj[i]) {
            if(scc[i] != scc[v]) condensed[scc[i]].insert(scc[v]);
        }
    }
    ...
    function<LL(int)> dfs3 = [&](int u) {
        ...
        for(int v: condensed[u]) {
            if(!vis[v]) mx = max(mx, dfs3(v));
            else mx = max(mx, dp[v]);
        }
        return dp[u] = mx + condensed_weights[u];
    };  
    ...
    LL mx = 0;
    rep(i,scc_count) mx = max(mx, dp[i]);
    putl(mx);
