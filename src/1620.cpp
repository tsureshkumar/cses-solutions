...
LL solveWithPQ() {
  priority_queue<pair<LL,LL>, vector<pair<LL,LL>>, greater<pair<LL,LL>>> pq;
    rep(i,n) pq.push({times[i], i});

    LL finishTime = 0;
    while(t--) {
        auto [ftime, id] = pq.top(); pq.pop();
        finishTime = max(finishTime, ftime);
        pq.push({ftime + times[id], id});
    }

    return finishTime;
}

bool can_make_x_prods_in_t_time(LL x, LL t) {
    LL possible = 0;
    rep(i,n) {
        int machine_time = times[i];
        possible += t/machine_time;
        if(possible >= x) return true;
    }
...
LL solveWithBS() {
    ...
    LL left = 0, right = (LL)times.back()*t, mid;

    while(left <= right) {
        mid = left + (right-left)/2;
        bool can_make = can_make_x_prods_in_t_time(t, mid);

        debug(left, right, mid, can_make);
