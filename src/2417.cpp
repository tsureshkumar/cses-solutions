...
int divcount[1000001];

// a number < 10^6 has at most 8 divisors. 
// use inclusion-exclusion to count the number of numbers that are not coprime to a given number
// by maintaining count of indexes that are divisible by each divisors.
// for each prime factor, add count and for each combination subtract count. (PIE)
// complexity: O(N*8*2^8)
...
int main(int argc, char **argv) {
    ...
    rep(i, n) {
        ...
        for(int j=1; j<(1<<P); j++) {
            ...
            if(count&1) { // odd sets are added
                LL p = 1;   
                for(int k=0; k<P; k++) {
                    if(j&(1<<k)) p *= primes[k];
                }
                // subtract 1 because we don't want to count the number itself
                not_coprimes += (divcount[p]-1);
            } else { // even sets are subtracted
                LL p = 1;
                for(int k=0; k<P; k++) {
                    if(j&(1<<k)) p *= primes[k];
                }
        ...
        debug(i, a[i], not_coprimes, n-1-not_coprimes);

        ans += n-1-not_coprimes;
    }
    putl(ans/2);
