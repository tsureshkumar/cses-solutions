...
int main(int argc, char **argv) {
    ...
    while(n--) {
        ...
        LL left, right, lpos, rpos, p=1, digits=1;
        for(left=1, right=p*10, lpos=1, rpos=right; !(lpos <= k && k < rpos); ) {
            p *= 10;
            digits++;
            left = p;
            right = left*10;
            lpos = rpos;
            rpos = lpos + (right-left)*digits;
        }

        LL nn = left+(k-lpos)/digits;
        LL pos = digits-(k-lpos)%digits-1;

        debug(n, k, left, right, lpos, rpos, p, digits, nn, pos);
