...
int main(int argc, char **argv) {
    ...
    // n pearls and m colors
    int n,m; see(n,m);

    auto burnside_lemma = [&](int n, int m) -> LL { 
        // base case: 0 rotations. m^n neclaces remain the same
        LL ans = modpow(m, n, MD);

        repr(k, 1, n) {
            // for each k rotations, gcd(k,n) neclaces remain the same
            // for k rotation, a neclace of length n makes gcd(k,n) cycles
            // each of length n/gcd(k,n)
            int g = gcd(k, n);
            (ans += modpow(m, g, MD)) %= MD;
        }
        ans *= inv(n, MD);
        (ans += MD) %= MD;
    ...
    LL ans = burnside_lemma(n, m);
    putl(ans);

    // repr(i, 1, 11) {
    //     debug(n, i, burnside_lemma(n, i));
    // }
