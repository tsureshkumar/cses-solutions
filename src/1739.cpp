...
struct FenwickTree {
    int n;
    vvi bit;
    FenwickTree (int n) : n (n), bit (n+1, vi (n+1)) {}
...
int main(int argc, char **argv) {
    ...
    rep (_, q) {
        ...
        if (t == 1) {
            int r, c; see(r, c);
            // debug ("update?", r-1, r-1, c-1, c-1);
            int old = grid[r-1][c-1];
            ft.update (r, c, old ? -1 : 1);
            grid[r-1][c-1] = old^1;
        } else {
            int t1, b1, l1, r1; see(t1, l1, b1, r1);
            // debug ("query?", t1-1, b1-1, l1-1, r1-1);
            putl (ft.query (b1, r1) - ft.query (t1-1, r1) - ft.query (b1, l1-1) + ft.query (t1-1, l1-1));
        }
    }
