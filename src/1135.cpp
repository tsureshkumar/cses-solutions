...
int main(int argc, char **argv) {
    ...
    vi depth(n, INF);
    depth[0] = 0;
    rep(i,n) jump[i][0] = par[i], depth[i] = min(depth[i], depth[par[i]] + 1);
    repr(j,1,20) {
        rep(i,n) {
            jump[i][j] = jump[jump[i][j-1]][j-1];
            depth[i] = min(depth[i], depth[jump[i][j]] + (1<<(j)));
        }
    }
    auto lca = [&](int u, int v) {
        if(depth[u] < depth[v]) swap(u,v);
        int k = abs(depth[u] - depth[v]);
        dwnr(i,0,20) if(k & (1<<i)) u = jump[u][i];
        if(u == v) return u;
        dwnr(i,0,20) if(jump[u][i] != jump[v][i]) u = jump[u][i], v = jump[v][i];
        return jump[u][0];
    };
    ...
    while(q--) {
        ...
        --u; --v;
        int l = lca(u,v);
        putl(depth[u] + depth[v] - 2*depth[l]);
    }
