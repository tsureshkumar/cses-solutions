...
LD dp[102][605]; // dp[i][j] = probability of getting sum j after i throws
...
int main(int argc, char **argv) {
    ...
    repr(i,1,102) { // throw
        repr(j,1,605) { // sum
            repr(k,1,7) { // cur throw value
                if(j-k >= 0) {
                    // prob(i,j) = prob(i-1,j-k) * prob(k)
    ...
    LD sum = 0;
    repr(i,a,b+1) sum += dp[n][i];

    printf("%0.6Lf\n", sum);

    #if DEBUG==1
        rep(i,20) setw(3),put(i); putl("");
        rep(i,20) setw(3),put(dp[n][i]); putl("");
        debug(sum);
    #endif
