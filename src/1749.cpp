...
int main(int argc, char **argv) {
    ...
    auto update = [&](int p, int v) {
        p += M;
        segtree[p] = v;
        for(p /= 2; p > 0; p /= 2) segtree[p] = segtree[2*p] + segtree[2*p+1];
    };

    function<int(int,int,int,int)> query = [&](int i, int l, int r, int k) {
        if(segtree[i] < k+1) return -1;

        if(l == r) return l;
        int m = (l+r)/2;
        int res = query(2*i, l, m, k);
        if(res != -1) return res;
        return query(2*i+1, m+1, r, k-segtree[2*i]);
    };
