...
int main(int argc, char **argv) {
    ...
    vector<ii> seg1(2*N, {INF, INF}), seg2(2*N, {INF, INF});

    auto combine = [&](int i, vii &seg) {
        return min(seg[2*i], seg[2*i+1]);
    };

    auto update = [&](int pos, int v, vii &seg) {
        seg[pos+N] = {v, pos};
    ...
    auto query = [&](int l, int r, vii &seg) {
        ii res = {INF, INF};
        for(l += N, r += N; l < r; l /= 2, r /= 2) {
            if(l&1) res = min(res, seg[l++]);
            if(~r&1) res = min(res, seg[r--]);
        }
        if(l == r) res = min(res, seg[l]);
        return res;
    };

    rep(i, n) update(i, a[i]-i, seg1);
    rep(i, n) update(i, a[i]+i, seg2);

    auto printseg = [&](vii &seg) {
        #if DEBUG == 1
        rep(i, 2*N) {
            if (i == BitLSOne(i)) cout << "\n";
    ...
    rep(i, q) {
        ...
        if(c == 1) {
            int v; see(v);
            update(x-1, v-x+1, seg1);
            update(x-1, v+x-1, seg2);
            printseg(seg1);
            printseg(seg2);
        } else {
            ii left = query(0, x-1, seg1);
            ii right = query(x-1, n-1, seg2);
            debug(left.fi, left.se, right.fi, right.se);
            auto ans = min(left.fi+left.se+abs(left.se-x+1), right.fi-right.se+abs(right.se-x+1));
            putl(ans);
        }
    }
