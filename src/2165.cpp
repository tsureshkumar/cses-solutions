...
int main(int argc, char **argv) {
    ...
    vii moves;
    function<LL(int,int,int,int)> toh = [&](int n, int from, int to, int use) {
        if(n==0) return 0LL;
        if(n==1) { moves.pb({from, to}); return 1LL; } 

        LL res = 1;
        LL other = toh(n-1, from, use, to);
        moves.pb({from, to});
        other += toh(n-1, use, to, from);
        return other+res;
    };

    putl(toh(n, 1,3,2));
    for(auto x: moves) putl(x.fi, x.se);
