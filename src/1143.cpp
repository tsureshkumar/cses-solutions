...
int main(int argc, char **argv) {
    ...
    vi hotels(n); seea(hotels, 0, n);
    vi groups(m); seea(groups, 0, m);
    ...
    auto update = [&](int p, int v) {
        p += M;
        segtree[p] = v;
        for(p /= 2; p > 0; p /= 2) segtree[p] = max(segtree[2*p], segtree[2*p+1]);
    };

    function<int(int,int,int,int)> binsearch = [&](int i, int l, int r, int p) {
        if(segtree[i] < p) return -1; // not possible

        if(l == r) return segtree[i] >= p ? l : -1;
        int m = (l+r)/2;
        int res = binsearch(2*i, l, m, p);
        if(res != -1) return res;
        return binsearch(2*i+1, m+1, r, p);
    };

    rep(i, n) update(i, hotels[i]);

    rep(i, m) {
        int p = groups[i];
        int res = binsearch(1, 0, M-1, p);
        if(res == -1) put(0);
        else {
            put(res+1);
            hotels[res] -= p;
            update(res, hotels[res]);
        }
    }
    putl("");
