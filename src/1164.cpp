...
int main(int argc, char **argv) {
    ...
    // Read input
    int n; see(n);
    vii evs;    // events: (start, end)
    rep(i,n) {
        int s,e; see(s,e);
        evs.pb({s,e});
    }

    // Create and sort indices by start time
    vi idx(n); 
    iota(all(idx), 0);
    sort(all(idx), [&](int i, int j) {
        return evs[i].fi != evs[j].fi ? 
               evs[i].fi < evs[j].fi : 
               evs[i].se < evs[j].se;
    });

    // Assign rooms using priority queue
    vi rooms(n);
    int cnt = 0;  // room counter
    priority_queue<ii, vii, greater<ii>> pq;  // (end_time, room_num)
