...
int main(int argc, char **argv) {
    ...
    repr(i,1,m) {
        ...
        while(inboundi(i+dp[i], 0, m) && inboundi(i-dp[i], 0, m) && t[i+dp[i]] == t[i-dp[i]]) dp[i]++;
        if(i+dp[i] > r) l = i-dp[i], r = i+dp[i];
        #if DEBUG==1
        debug(i);
        rep(i,m) setw(3), put(i == r ? "p": i == i-dp[i] ? "l" : i==r ? "r" : " "); putl("");
        rep(i,m) setw(3), put(i); putl("");
        rep(i,m) setw(3), put(dp[i]); putl("");
        rep(i,m) setw(3), put(t[i]); putl("");
        putl("==");
        #endif
    }
    ...
    int mxi = 0;
    rep(i,m) if(dp[i] > dp[mxi]) mxi = i;
    string res = "";
    repr(l, mxi-dp[mxi]+1, mxi + dp[mxi]) {
        if(t[l] != '#') res += t[l];
    }
    putl(res);
