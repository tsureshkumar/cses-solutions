...
int main(int argc, char **argv) {
    ...
    sort(all(times), [&](ii i, ii j) {
        if(i.se == j.se) return i.fi < j.fi;
        return i.se < j.se;
    } );

    LL res = 0;
    multiset<int> persons;
    rep(i, k) persons.insert(0);

    rep(i, n) {
        auto [start, end] = times[i];

        auto it = persons.upper_bound(start);
        if(it == persons.begin()) continue;
        it--;
