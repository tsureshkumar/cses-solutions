...
void bottomupSolve(int n, int x, vi &price, vi &pages) {
    vvi dp(n+1, vi(MX, -1));

    vi cumpages(n+1, 0), cumprice(n+1, 0);
    dwn(i, n) cumpages[i] = cumpages[i+1] + pages[i];
    dwn(i, n) cumprice[i] = cumprice[i+1] + price[i];

    function<int(int, int)> dfs = [&](int i, int rem) -> int {
        if(i == n) return rem >= 0 ? 0 : -INF;
        if(rem == 0) return 0;
        if(rem < 0) return -INF;
        if(rem >= cumprice[i]) return cumpages[i];

        int &res = dp[i][rem];
        if(res != -1) return res;

        return res = max(dfs(i+1, rem), dfs(i+1, rem-price[i]) + pages[i]);
    };
...
void topdownSolve(int n, int x, vi &price, vi &pages) {
...
int main(int argc, char **argv) {
    ...
    //bottomupSolve(n, x, price, pages);
    topdownSolve(n, x, price, pages);
