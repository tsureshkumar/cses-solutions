...
int main(int argc, char **argv) {
    ...
    vvl dp(b+2, vl(b+2, INF));


    rep(i,b+1) dp[i][1] = dp[1][i] = i-1;
    rep(i,b+1) dp[i][i] = dp[i][i] = 0;

    repr(i,2,b+1) {
        repr(j,i+1,b+1) {
            dwnr(k,j/2,j) {
                dp[j][i] = dp[i][j] = min(dp[i][j], 1 + dp[i][k] + dp[i][j-k]);
            }

            dwnr(k,i/2,i) {
                dp[j][i] = dp[i][j] = min(dp[i][j], 1 + dp[j][k] + dp[j][i-k]);
            }
        }
    }

    // int K = 10;
    // setw(3); put("*"); rep(j,K+1) { setw(3);put(j); } putl();
    // rep(i,K+1) {
    //     setw(3);put(i);
    //     rep(j,K+1) {
    //         setw(3);put(dp[i][j] == INF ? -1 : dp[i][j]);
    //     }
    //     putl();
    // }   
