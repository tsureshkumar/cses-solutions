...
int main(int argc, char **argv) {
    ...
    for(int k=1; k<n; k*=2) {
        vii pairs(n);
        rep(i,n) pairs[i] = {ra[i], i+k < n ? ra[i+k] : 0};
    ...
    vi lcp(n), prev_by_sa(n), plcp(n);
    prev_by_sa[sa[0]] = -1;
    repr(i,1,n) prev_by_sa[sa[i]] = sa[i-1];
    int l = 0;
    rep(i,n) {
        if(prev_by_sa[i] == -1) {
            plcp[i] = 0;
    ...
    #if DEBUG == 1
    //rep(i,n) putl(i, sa[i], ra[i], plcp[i], lcp[i], t.substr(sa[i]));
    #endif
