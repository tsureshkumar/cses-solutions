...
int main(int argc, char **argv) {
    ...
    rep(i, n) if(!vis[i]) dfs1(i, -1);

    vi roots(n, -1);
    function<void(int, int, int)> dfs2 = [&](int u, int p, int root) {
        if(vis[u]++) return;
        roots[u] = root;
        rep(i, rev_adj[u].size()) {
            int v = rev_adj[u][i];
            if(v == p) continue;
            dfs2(v, u, root);
        }
    ...
    while(!st.empty()) {
        int u = st.top(); st.pop();
        if(!vis[u]) {
            if(a == -1) a = u; // first component
            else b = u; // second component
            dfs2(u, -1, u);
        }
    }

    rep(i, n) debug(i, roots[i]);

    set<int> s;
    rep(i, n) s.insert(roots[i]);
    if(s.size() == 1) {
        putl("YES");
        return 0;
    } else {
