...
int main(int argc, char **argv) {
    ...
    // First pass: Find segments that contain other segments
    {
        ...
        dwn(i, n) {
            ...
            auto it = end_points.lower_bound(curr.se);
            if(it != end_points.begin() || (it != end_points.end() && *it == curr.se)) {
                contains[segids[i]] = 1;
            }
            end_points.insert(curr.se);
        }
    }
    ...
    // Second pass: Find segments that are contained in other segments
    {
        ...
        rep(i, n) {
            ...
            auto it = end_points.lower_bound(curr.se);
            if(it != end_points.end() && *it >= curr.se) {
                contained_in[segids[i]] = 1;
            }
            end_points.insert(curr.se);
        }
    }
