...
int main(int argc, char **argv) {
    ...
    vii dp(1<<n, {n, 0}); // {max trips, min weight}

    dp[0] = {0, x};

    for(int subset = 1; subset < (1<<n); subset++) {
        int remaining = subset;
        while(remaining > 0) {
            int idx = __builtin_ctz(remaining);
            int w = v[idx];
            int others = subset ^ (1<<idx);


            // if we can add this weight to the subset, do it
            if(dp[others].se + w <= x) {
                dp[subset] = min(dp[subset], {dp[others].fi, dp[others].se + w});
            } else { // we have to start a new trip
                dp[subset] = min(dp[subset], {dp[others].fi + 1, w});
            }

            // stringstream ss;
            // ss << bitset<10>(subset) << " " << bitset<10>(others);
            // debug(idx, w, subset, dp[subset].fi, dp[subset].se, ss.str());
