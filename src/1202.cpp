...
int main(int argc, char **argv) {
    ...
    struct data {
        LL cost, u, routes, min_flights, max_flights, version;
    ...
    vector<data> cost(n, {INF, 0, 0, INF, 0, 0});
    cost[0] = {0, 0, 1, 0, 0, 0};
    vi vis(n, 0);

    while(!pq.empty()) {
        auto cur = pq.top(); pq.pop();
        debug(cur.cost, cur.u+1, cur.routes, cur.min_flights, cur.max_flights, cur.version);
        if(vis[cur.u]) continue;
        vis[cur.u] = 1;

        for(auto [v, c2]: adj[cur.u]) {
            if(cost[v].cost >= cost[cur.u].cost + c2) {
                LL new_cost = cost[cur.u].cost + c2;
                LL routes = (cost[v].cost == new_cost ? cost[v].routes + cost[cur.u].routes : cost[cur.u].routes) % MD;
                LL min_flights = (cost[v].cost == new_cost ? min(cost[v].min_flights, cost[cur.u].min_flights+1) : cost[cur.u].min_flights+1);
                LL max_flights = (cost[v].cost == new_cost ? max(cost[v].max_flights, cost[cur.u].max_flights+1) : cost[cur.u].max_flights+1);
                cost[v] = {new_cost, v, routes, min_flights, max_flights, cur.version+1};
                pq.push(cost[v]);
            }
        }
    }

    putl(cost[n-1].cost, cost[n-1].routes, cost[n-1].min_flights, cost[n-1].max_flights);
