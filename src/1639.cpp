...
int main(int argc, char **argv) {
    ...
    dwnr(i,0,n+1) {
        dwnr(j,0,m+1) {
            if (i < n && j < m && line[i] == target[j]) {
                dp[i][j] = dp[i+1][j+1];
            } else {
                dp[i][j] = min(dp[i][j], INSERT + dp[i][j+1]);
                dp[i][j] = min(dp[i][j], DELETE + dp[i+1][j]);
                dp[i][j] = min(dp[i][j], REPLACE + dp[i+1][j+1]);
            }
        }
    }

    // setw(3); put("*"); rep(j,m+1) { setw(3);put(j); } putl();
    // rep(i,n+1) {
    //     setw(3);put(i);
    //     rep(j,m+1) {
    //         setw(3);put(dp[i][j]);
