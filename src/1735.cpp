...
struct LazySegmentTree {
    ...
    vl tree, lazy;
    vi hardset; // hardset[v] = 1 if the node has been set, 0 otherwise
    LazySegmentTree (int n) {
        tree.resize (4*n+1, 0);
        lazy.resize (4*n+1, 0);
        hardset.resize (4*n+1, 0);
    }
    void push (int v, int tl, int tr) {
        if (hardset[v] == 1) {
            tree[v] = lazy[v] * (tr-tl+1); // then apply if any pending lazy that happened after hardset
            if (tl != tr) {
                hardset[v*2] = hardset[v];
                hardset[v*2+1] = hardset[v];
                lazy[v*2] = lazy[v];
                lazy[v*2+1] = lazy[v];
            }
            lazy[v] = 0;
    ...
    void update (int v, int tl, int tr, int l, int r, int add, bool isHardset = false) {
        push (v, tl, tr);
        if (tr < l || tl > r) return;
        if (l <= tl && tr <= r) {
            lazy[v] += add;
        ...
        int tm = (tl + tr) / 2;
        update (v*2, tl, tm, l, min(r, tm), add, isHardset);
        update (v*2+1, tm+1, tr, max(l, tm+1), r, add, isHardset);
...
int main(int argc, char **argv) {
    ...
    rep (_, q) {
        int t; see(t);
        if (t == 1) { // increase by x for the range [a,b]
            int a, b, x; see(a, b, x);
            // debug("update", a, b, x);
            st.update (1, 0, N-1, a-1, b-1, x);
        } else if (t == 3) { // query the range [a,b]
            int a, b; see(a, b);
            putl (st.query (1, 0, N-1, a-1, b-1));
        } else { // set to x in the range [a,b]
            int a, b, x; see(a, b, x);
            //debug("set", a, b, x);
            st.set (1, 0, N-1, a-1, b-1, x);
        }
    }
