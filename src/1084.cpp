...
int N, M, K;
vi apt2person, prefs, avails;

int bipartite_match(int i) {
    int ask = prefs[i];

    debug("finding for ", i);

    auto left = lower_bound(all(avails), ask-K);
    auto right = lower_bound(all(avails), ask+K);

    int l = distance(avails.begin(), left); 
    int r = distance(avails.begin(), right); 
    ...
    for(int k=l;
            ...
            k++) {
        int price = avails[k];
        if(ask-K <= price && price <= ask+K) {
            if(apt2person[k] < i) {
                int old = apt2person[k];
                apt2person[k] = i;
                debug("asking ", old, "to vacate");
                if(bipartite_match(old)) {
                    debug(old, "vocated");
                    return 1;
                } else {
                    apt2person[k] = old;
                }
            } else if (apt2person[k] >= N) {
                apt2person[k] = i;
                debug("found home for", i, "with", k);
                return 1;
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    apt2person.assign(M, N);

    debug(N, M);

    //int c =0;
    //rep(i,N) {
    //    int matching = bipartite_match(i);
    //    if(matching) c++;
    //}
    ...
    while(i < N && j < M) {
        ...
        if(ask-K <= price && price <= ask+K) {
            i++, j++, c++;
        } else if(price < ask-K) j++;
        else if(price > ask+K) i++;
    }
    putl(c);
