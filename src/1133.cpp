...
int main(int argc, char **argv) {
    ...
    // calculate subtree sizes and sum to root
    function<void(int, int, int)> dfs = [&](int u, int p, int d) {
        par[u] = p;
        dp[0].subtree_sum += d; // sum to root
        dp[u].subtree_size = 1;
        for(int v: adj[u]) {
            if(v == p) continue;
            dfs(v, u, d+1);
            dp[u].subtree_size += dp[v].subtree_size;
        }
    };
    dfs(0, -1, 0);
    ...
    function<void(int, int)> dfs2 = [&](int u, int p) {
        for(int v: adj[u]) {
            if(v == p) continue;
            // rerooting technique
            // when rerooting from u to v, nodes in v's subtree will be at +1 distance from root
            // and all other nodes will be at -1 distance from root
            // and hence the distance lead to the following formula
            dp[v].subtree_sum = dp[u].subtree_sum - dp[v].subtree_size + (n - dp[v].subtree_size);
            dfs2(v, u);
        }
    };
    dfs2(0, -1);

    rep(i, n) put(dp[i].subtree_sum);
    putl("");
