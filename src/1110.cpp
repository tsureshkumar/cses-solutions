...
vi tempFa;
   ...
   int main(int argc, char **argv) {
      ...
      vi hashes(m+1), pows(m+1) ;
      const int A = 31, B= (int)1e9+9;
      hashes[0] = t[0]-'a', pows[0] = 1;
      rep(i,m) {
        hashes[i+1] = ((LL)hashes[i] * A + t[i+1]-'a') % B;
        pows[i+1] = (LL)pows[i] * A % B;
      }

      auto getHash = [&](int l, int r) {
        // res will have A^l * a_l + A^(l-1) * a_l+1 + ... + A^0 * a_r
        return (LL) (hashes[r] - (l > 0 ? (LL)hashes[l-1] * pows[r-l+1] % B : 0) + B) % B;
      };

      int smallest =  0;
      for(int j=1; j<n; j++) {
        debug(j, j+n-1, smallest, getHash(j, j+n-1), getHash(smallest, smallest+n-1));
        int l=j, r=j+n-1, mid;
        while(l <= r) {
          mid = l + (r-l)/2;
          debug(l, mid, r, getHash(l,mid), getHash(smallest + l-j, smallest+(mid-j)));

          if(getHash(l,mid) == getHash(smallest + l-j, smallest+(mid-j)))
            l = mid+1;
          else
            r = mid-1;
        }
        debug(l, j+n, t[l], t[smallest+l-j]);
        if(l < j+n && t[l] < t[smallest+l-j]) smallest = j;
      }
      debug(smallest);
      putl(t.substr(smallest, n));
