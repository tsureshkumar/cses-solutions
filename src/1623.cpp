...
LL res=INF;
void dfs(vi &xs, int i, LL sum, LL total) {
    if(i == xs.sz) { res = min(res, abs(2*sum - total)); return ; }
    dfs(xs, i+1, sum, total);
    dfs(xs, i+1, sum+xs[i], total);
}
...
int main(int argc, char **argv) {
    ...
    auto gray = [](int n) { return n ^ (n>>1); };

    LL total = accumulate(all(xs), 0LL);
    LL s = 0;
    int prev = 0;
    rep(i,1<<n) {
        int gc = gray(i);
        int ppos = gc^prev;
        int pos = ppos ? __builtin_ctz(ppos) : -1;
        if(pos >= 0) {
            if(gc&ppos) s += xs[pos];
            else s -= xs[pos];
        }
        res = min(res, abs(2*s-total));
        debug(i, gc, ppos, pos, s, 2*s-total);
        prev = gc;
    }
