...
int main(int argc, char **argv) {
    ...
    while(!pq.empty()) {
        ...
        //if((LL)dist[u].size() < k || *dist[u].rbegin() < d) continue;
        if(dist[u].find(d) == dist[u].end()) continue;

        for(auto [v,w]: adj[u]) {
            if((LL)dist[v].size() < k || *dist[v].rbegin() > d+w) {
                dist[v].insert(d+w);
                while((LL)dist[v].size() > k) dist[v].erase(--dist[v].end());
                pq.push({d+w,v});
            }
        }
    }

    for(auto d: dist[n-1]) put(d);
    cout << "\n";
