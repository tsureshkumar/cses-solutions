...
int main(int argc, char **argv) {
    ...
    for(auto [a,b,p] : v) {
        if(!v2.empty() && get<1>(v2.back()) == b && get<0>(v2.back()) == a && get<2>(v2.back()) <= p) {
            v2.pop_back();
        }
        v2.pb({a,b,p});
    }
    ...
    for(int i=0; i<n; i++) {
        ...
        auto it = upper_bound(all(v), tuple<int,int,int>{0,a,0}, 
            [](tuple<int,int,int> a, tuple<int,int,int> b) {
            return get<1>(a) <= get<1>(b);
        });
        int j = it - v.begin() - 1;
        if(j >= 0) {
            best[i] = max(best[i-1], (LL)p + best[j]);
        } else {
            best[i] = max(i>0 ?best[i-1] : 0, (LL)p);
        }
        debug(i, j, a,b,p, best[i]);
    }
