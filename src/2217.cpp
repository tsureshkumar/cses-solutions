...
int main(int argc, char **argv) {
    ...
    int res = 1;
    rep(i,n) {
        if(pos[xs[i]] > pos[xs[i]-1]) continue;
        res++;
    }
    ...
    while(m--) {
        ...
        vector<int> affected_nos;

        vi candidates = { xs[x], xs[y], xs[x]+1, xs[y]+1 };
        ...
        for(auto x: affected_nos) {
            debug(x);
        }

        vi inplace(affected_nos.sz, false);
        vi new_inplace(affected_nos.sz, false);

        rep(i,affected_nos.sz) {
            int x = affected_nos[i];
            bool xinpos = pos[x] > pos[x-1];
            debug(x, pos[x], pos[x-1], xinpos);
            inplace[i] = xinpos;
        }

        swap(pos[xs[x]], pos[xs[y]]);
        swap(xs[x], xs[y]);

        rep(i,affected_nos.sz) {
            int x = affected_nos[i];
            bool xinpos = pos[x] > pos[x-1];
            debug("later swap", x, pos[x], pos[x-1], xinpos);
            new_inplace[i] = xinpos;

            debug(x, pos[x], inplace[i], new_inplace[i]);
        }


        rep(i,affected_nos.sz) {
            bool xinpos=inplace[i], new_xinpos = new_inplace[i];
            if(xinpos && !new_xinpos) res++;
            if(!xinpos && new_xinpos) res--;
        }
        putl(res);
    }
