...
public:
    ...
    void update(int i, int v) {
        ++i; // 1-indexed
        for(; i<=n; i+=BitLSOne(i)) tree[i] += v;
    }
    LL query(int i) {
        ++i; // 1-indexed
        LL sum = 0;
        for(; i>0; i-=BitLSOne(i)) sum += tree[i];
        return sum;
    }
};
...
int main(int argc, char **argv) {
    ...
    rep(i,q) {
        ...
        if(c == 1) {
            ...
            int diff = v-a[u];
            a[u] = v;
            ft.update(order[u], diff);
        } else {
            int x; see(x); --x;
            int l = order[x], r = order[x]+tree_size[x]-1;
            putl(ft.query(r)-ft.query(l-1));
        }
    }
