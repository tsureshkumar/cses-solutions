...
#define debugv(v) {cout << HIGHLIGHT; for(__typeof(v[0]) &x:v) { debug(x); } cout << std::endl; cout << RESETHIGH;}
#else
...
public:
    ...
    set<int> components() {
        set<int> comps;
        rep(i, p.size()) comps.insert(find(i));
        return comps;
    }
};
...
int main(int argc, char **argv) {
    ...
    putl(comps.size()-1);
    for(auto it = next(comps.begin()); it != comps.end(); it++) {
        putl(*(prev(it))+1, *it+1);
    }
