...
int main(int argc, char **argv) {
    ...
    int n, q; see(n, q);
    vi a(n+1, 0); seea(a, 1, n+1);

    // build binary jump table
    vvi jump(n+1, vi(30, 0));
    rep(i, n+1) jump[i][0] = a[i];
    repr(j, 1, 30) {
        rep(i, n+1) {
    ...
    // setw(3); put("*"); rep(i,32) { setw(3); put(i); } putl("");
    // rep(i,n+1) {
    //     setw(3); put(i);
    //     rep(j,32) {
    //         setw(3); put(jump[i][j]);
    ...
    while(q--) {
        ...
        while(k > 0) {
            int least_sig_bit = __builtin_ctz(k);
            debug(x,k,least_sig_bit);
            x = jump[x][least_sig_bit];
            k = k & (k-1);
        }
        putl(x);
    }
