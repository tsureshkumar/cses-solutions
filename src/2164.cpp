...
int main(int argc, char **argv) {
    ...
    // josephus problem with every 2nd person eliminated.
    // every index eliminated is 2*k for some k in {0, 1, 1, 2, 2, 3, 3, 4, 4, ...}.
    // every index progressing to next cycle is 2*k+1. The progressing one gets a new index
    // of the form n + k + 1.  In next cycle, the process continues. i.e. n+k+1 is now treated
    // as 2*k1 or 2*k1+1 for some k1. If 2*k1, eliminated. If 2*k1+1, progressing to next cycle as
    // n+k1+1. This continues. 
    // To find kth eliminated, its index should have been 2*k = N. To find the original index under n,
    // first find the previous cycle's N. Since it survived previous cycle, it is also of the form N' = 2*k1+1.
    // repeating this until N' < n, we get the original index.
    auto solve = [](int n, int k) -> LL {
        LL N = 2*k;
        while(N > n) {
            LL k = N-n-1;
            N = 2*k+1;
