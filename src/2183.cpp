...
int main(int argc, char **argv) {
    ...
    rep(i, n) {
        // invariant:
        // upto i, we will be able to make an amount of last
        // if we add x, because we can make from 1 to last,
        // we can make s,s-1,...s-last.  So if we can cover
        // from last+1 upto s, then we are good.
        int x = xs[i];
        cumsum += x;
        int from = cumsum - last;
        if(from <= last+1)  {
            last = cumsum;
            continue;
        } else {
