...
int main(int argc, char **argv) {
    ...
    auto burnside_lemma = [&](LL n) -> LL {
        LL ans = 0;

        // 4 rotations (identity, 90, 180, 270)
        // under each rotation, the cells form a cycle groups.
        // For ex, 90 rotation, the corner cells form  one group.
        // Similarly, 180 rotation, (i,j) and (n-i+1,n-j+1) form a group.
        // effectively, counting quadrants, halfs.
        int d[] = {1, 4, 2, 4};
        rep(i, 4) {
            // fermat's little theorem for large exponents
            // by adding d[i]-1 round up, we include the axis 
            // cells too in case n is odd.
            LL exp = ((n*n + d[i]-1)/d[i]) % (MD-1);
            ans += modpow(2, exp, MD);
        }

        // this problem especially asks only for rotational symmetries C4.
