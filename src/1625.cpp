...
void nav(int i, int j, int pathidx, int lvl) {
    if(grid[i][j] <= lvl) {
        return ;
    }
    if(i==7 && j==1) {
    ...
    // loop deads
    if(grid[i-1][j] <= lvl && grid[i+1][j] <= lvl && grid[i][j-1] > lvl && grid[i][j+1] > lvl) return;
    if(grid[i][j-1] <= lvl && grid[i][j+1] <= lvl && grid[i-1][j] > lvl && grid[i+1][j] > lvl) return;
    ...
    if(path[pathidx] == 'U') 
        nav(i-1, j+0, pathidx+1, lvl+1);
        //nav(i + dx[k], j+dx[k+1], pathidx+1, lvl+1);
    else if(path[pathidx] == 'D') 
        nav(i+1, j+0, pathidx+1, lvl+1);
    else if(path[pathidx] == 'R') 
        nav(i+0, j+1, pathidx+1, lvl+1);
    else if(path[pathidx] == 'L') 
        nav(i+0, j-1, pathidx+1, lvl+1);
    else {
        //rep(k,4) {
            //nav(i + dx[k], j+dx[k+1], pathidx+1, lvl+1);
...
int main(int argc, char **argv) {
    ...
    rep(i,9) grid[0][i] = grid[i][0] = grid[8][i] = grid[i][8] = 0; 
